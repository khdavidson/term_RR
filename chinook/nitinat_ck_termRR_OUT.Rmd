...
![Nitinat terminal run reconstruction study area](C:/Users/DAVIDSONKA/Documents/Notes, records, resources/Area maps/statarea21_maponly.pdf "Nitinat terminal run reconstruction study area"){width=100% height=700}
```{r studyarea-fig, fig.cap='Nitinat run reconstruction study area.'}
knitr::include_graphics(here::here("data", "statarea21_maponly.png"))
knitr::include_graphics(here::here("data", "statarea22_maponly.png"))
```
...
leaflet() %>% 
  ...
  addCircleMarkers(data=bioConFull%>%filter(!is.na(lat) & YEAR==RR_year)%>%
...
leaflet() %>% 
  ...
  addCircleMarkers(data=catConFull%>%filter(YEAR==RR_year)%>%group_by(sector,RUN_RECON_AREA, lat,long,label)%>%
    ...
    color=~pal(sector), stroke=F,  fillOpacity=0.8, label=~htmlEscape(label)) %>%
  # rec biosample markers ---
  addCircleMarkers(data=bioConFull%>%filter(!is.na(lat) & YEAR==RR_year-1)%>%
                     group_by(YEAR,sector,RUN_RECON_AREA,CREEL_SUB_AREA,lat,long)%>%summarize(n=n(),label=unique(label)),
    radius=~ifelse(n<10, 3, ifelse(n>=10&n<50, 6, ifelse(n>=50&n<75, 8, ifelse(n>=75&n<100, 10, 12)))),
    color="green", stroke=T, fillOpacity=0, label=~htmlEscape(label)) 
```
...
leaflet() %>% 
  ...
  addCircleMarkers(data=catConFull%>%filter(YEAR==RR_year)%>%group_by(sector,RUN_RECON_AREA, lat,long,label)%>%
    ...
    color=~pal(sector), stroke=F,  fillOpacity=0.8, label=~htmlEscape(label)) %>%
  # rec biosample markers ---
  addCircleMarkers(data=bioConFull%>%filter(!is.na(lat) & YEAR==RR_year-2)%>%
                     group_by(YEAR,sector,RUN_RECON_AREA,CREEL_SUB_AREA,lat,long)%>%summarize(n=n(),label=unique(label)),
    radius=~ifelse(n<10, 3, ifelse(n>=10&n<50, 6, ifelse(n>=50&n<75, 8, ifelse(n>=75&n<100, 10, 12)))),
    color="green", stroke=T, fillOpacity=0, label=~htmlEscape(label)) 
```
...
leaflet() %>% 
  ...
  addCircleMarkers(data=catConFull%>%filter(YEAR==RR_year)%>%group_by(sector,RUN_RECON_AREA, lat,long,label)%>%
    ...
    color=~pal(sector), stroke=F,  fillOpacity=0.8, label=~htmlEscape(label)) %>%
  # rec biosample markers ---
  addCircleMarkers(data=bioConFull%>%filter(!is.na(lat) & YEAR==RR_year-3)%>%
                     group_by(YEAR,sector,RUN_RECON_AREA,CREEL_SUB_AREA,lat,long)%>%summarize(n=n(),label=unique(label)),
    radius=~ifelse(n<10, 3, ifelse(n>=10&n<50, 6, ifelse(n>=50&n<75, 8, ifelse(n>=75&n<100, 10, 12)))),
    color="green", stroke=T, fillOpacity=0, label=~htmlEscape(label)) 
```
